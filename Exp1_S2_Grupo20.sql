...
DROP TABLE IF EXISTS detalle_factura CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS factura CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS detalle_boleta CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS boleta CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS producto CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS unidad_medida CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS pais CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS cliente CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS vendedor CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS forma_pago CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS banco CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS comuna CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS ciudad CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS tramo_antiguedad CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS tramo_escolaridad CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS remun_mensual_vendedor CASCADE CONSTRAINTS;
DROP TABLE IF EXISTS escolaridad CASCADE CONSTRAINTS;
...
/* ---- CASO 1 ---- */
SELECT numfactura AS "N° Factura",
       TO_CHAR(fecha, 'FMDD "de" Month "de" YYYY') AS "Fecha Emision",
       LPAD(rutcliente, 10, '0') AS "RUT Cliente",
       '$' || TO_CHAR(neto,'9G999G990') AS "Monto Neto",
       '$' || TO_CHAR(iva,'9G999G990') AS "Monto IVA",
       '$' || TO_CHAR(total,'9G999G990') AS "Total Factura",
       CASE
...
/* ---- CASO 2 ---- */
SELECT
    LPAD(rutcliente, 10, '*') AS "RUT Cliente",
    nombre AS Cliente,
    COALESCE(TO_CHAR(telefono), 'Sin Telefono') AS Telefono,
    COALESCE(TO_CHAR(codcomuna), 'Sin Comuna') AS Comuna,
    estado AS Estado,
    CASE
        WHEN (saldo / credito) < 0.5 THEN
            'Bueno: ' || TO_CHAR( (credito - saldo), '$' || '9G999G990')
        WHEN (saldo / credito) <= 0.8 THEN
            'Regular: ' || TO_CHAR( (saldo), '$' || '9G999G990')
        ELSE 'Critico'
    END AS "Estado Credito",
    COALESCE(SUBSTR(mail, INSTR(mail, '@') + 1), 'Sin Correo') AS Correo
    FROM CLIENTE
...
SELECT
    ...
    CASE
        WHEN valorcompradolar IS NULL THEN
            'Sin Registro'
        ELSE REGEXP_REPLACE(TO_CHAR(valorcompradolar) || ' USD ', ',' , '.')
    END AS "Compra en USD",
    CASE
        WHEN valorcompradolar IS NULL THEN
            'Sin Registro'
        ELSE '$' || TO_CHAR(valorcompradolar * &&CAMBIO_DOLAR, '9G999G990') || ' PESOS '
    END AS "USD Convertido",
    totalstock AS Stock,
    CASE
        WHEN totalstock IS NULL THEN
            'Sin Datos'
        WHEN totalstock < &&UMBRAL_BAJO THEN
            '¡STOCK BAJO!'
        WHEN totalstock BETWEEN &&UMBRAL_BAJO AND &&UMBRAL_ALTO THEN
            '¡Reabastecer Pronto!'
        ELSE 'OK'
    END AS "Alerta Stock",
    CASE
        WHEN totalstock > 80 THEN
            '$' || TO_CHAR(vunitario - (vunitario * 0.1), '9G999G990')
        ELSE 'N/A'
